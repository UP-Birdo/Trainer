...
> als Paket hochgeladen. Dazu: **`tests/`** (Regressionskette, Abschnitt 11), **`tools/`** (`icons.py` +
> Master-Icon `Trainer-Icon-1024.png`), **`docs/`** (dieses Dokument, `SIMPELHEIT.md`,
> `GITHUB-EINRICHTUNG.md`) und das Repo-Außenpaket im Wurzelordner (`README.md`, `CHANGELOG.md`,
> `LICENSE`, `.github/`).
>
...
| `tools/Trainer-Icon-1024.png` | Master-Icon (Quelle für die drei Größen; nur Tooling) |
| `tools/icons.py` | Erzeugt die drei Icon-Größen aus dem Master (Pfade relativ zum Skript, Ausgabe in die Wurzel; braucht Python + numpy/scipy/Pillow) |
| `tests/` | Regressionskette (test79–82 + extract.js + README.md; läuft lokal UND als GitHub Action, s. Abschnitt 11) |
| `docs/` | Interne Doku: dieses Dokument, `SIMPELHEIT.md` (Konzept), `GITHUB-EINRICHTUNG.md` (Klick-Anleitung) |
...
`python3 tools/icons.py` laufen lassen (findet Master und Zielordner relativ zu sich selbst),
`?v=` an `apple-touch-icon`/`manifest.json` in der `index.html` hochzählen,
...
* **Ordnerstruktur (2. Nachgang, 07/2026):** Interne Doku nach `docs/` (UEBERGABE, SIMPELHEIT,
  GITHUB-EINRICHTUNG — Dateinamen bewusst UNVERÄNDERT: `index.html`-Kommentare verweisen auf
  `SIMPELHEIT.md`, Umbenennen hieße App-Datei anfassen), Tooling nach `tools/` (`icons.py`-Pfade von
  hartkodiertem `/home/claude/…` auf relativ-zum-Skript umgestellt), `tests/LIESMICH.md` →
  `tests/README.md` (GitHub zeigt READMEs in Ordnern automatisch an). Die Deploy-Dateien bleiben
  UNVERSCHIEBBAR im Wurzelordner: `index.html` ist der Pages-Einstieg, `sw.js` wird unter genau
  diesem Namen registriert (anderer Pfad = anderer SW-Scope!), `manifest.json`/Icons sind aus der
  `index.html` verlinkt.
...
* **Vor dem Ausliefern testen.** Die AKTUELLE Kette liegt im Ordner **`tests/`** (test79–82 + `extract.js`,
  Anleitung in `tests/README.md`). Prinzip: die ECHTEN Funktionen werden per `grabFn()` aus der
...
1. Den kompletten Projektordner mitgeben: 6 Deploy-Dateien im Wurzelordner, `docs/` (UEBERGABE,
   SIMPELHEIT, GITHUB-EINRICHTUNG), `tools/` (icons.py + Master-Icon), `tests/` und das
   Repo-Außenpaket (`README.md`, `CHANGELOG.md`, `LICENSE`, `.github/`).
2. Diesen Satz mitschicken: *„Das ist mein Trainer-Projekt, lies docs\UEBERGABE.md **und docs\SIMPELHEIT.md** — wir machen dort weiter."*
3. Der nächste Claude liest **beide** Dokumente (erst UEBERGABE = Ist-Zustand, dann SIMPELHEIT =
   ...
   und weiterbauen. **Vor dem Ausliefern:** `tests/` laufen lassen (Anleitung in `tests/README.md` —
   ...
   Anleitung Schritt 6 in `docs/GITHUB-EINRICHTUNG.md`).
